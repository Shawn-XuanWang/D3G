...
class DGSys:
    ...
    def setVariables(self, robotswarm=None):
        if robotswarm is None or self.num_uav == 0:
            ...
            self.waypoints = [None] * self.num_uav
            self.obst_d = [None] * self.num_uav

            for i in range(self.num_uav):
                self.aux_var[i] = robotswarm.cost_auxvar[i]
                self.n_aux_var[i] = self.aux_var[i].numel()
                self.state_var[i] = robotswarm.states[i]
                self.n_state_var[i] = self.state_var[i].numel()
                self.state_bd[i] = robotswarm.states_bd[i]
                self.ctrl_var[i] = robotswarm.ctrl[i]
                self.n_ctrl_var[i] = self.ctrl_var[i].numel()
                self.waypoints[i] = robotswarm.waypoint[i]
                # self.obst_d[i] = robotswarm.obst_d[i]
    ...
    def setDyn(self, robotswarm):
        ...
        self.dynf = [None] * self.num_uav


        for i in range(self.num_uav):
            self.dyn[i] = robotswarm.states[i] + robotswarm.dt * robotswarm.dynf[i]
            self.dyn_fn[i] = casadi.Function('dynamics' + str(i),
                                             [self.state_var[i], self.ctrl_var[i]], [self.dyn[i]])
            self.dynf[i] = robotswarm.dynf[i]
    ...
    def setCosts(self, robotswarm):
        ...
        for i in range(self.num_uav):
            assert robotswarm.path_cost[i].numel() == 1, "path_cost must be a scalar function"
        ...
        for i in range(self.num_uav):
            self.path_cost[i] = robotswarm.path_cost[i]
            self.path_cost_fn[i] = casadi.Function('path_cost' + str(i),
                                                   [self.neighbor_state_var[i], self.ctrl_var[i], self.waypoints[i],
                                                    self.aux_var[i]],
                                                   [self.path_cost[i]])
            self.final_cost[i] = robotswarm.final_cost[i]
    ...
    def def_DPMP(self):
        ...
        self.state_var_next = [None] *self.num_uav
        self.path_Hamil = [None] * self.num_uav
        self.final_Hamil = [None] * self.num_uav
        self.path_Lagr = [None] * self.num_uav
        self.path_Hamil_Quad = [None] * self.num_uav

        for i in range(self.num_uav):
            self.costate[i] = casadi.SX.sym('lambda' + str(i), self.state_var[i].numel())
            self.state_var_next[i] = casadi.SX.sym('x_next' + str(i), self.state_var[i].numel())
            ...
            self.path_Hamil_Quad[i] = dot(self.path_Hamil[i].T, self.path_Hamil[i])

            self.path_Lagr[i] = self.path_cost[i] + dot(self.state_var_next[i] - (self.dyn[i]), self.costate[i])
        ...
        self.dGu = [None] * self.num_uav
        self.dGu_fn = [None] * self.num_uav
        ...
        for i in range(self.num_uav):
            ...
            self.dHx_fn[i] = casadi.Function('dHx' + str(i),
                                             [self.neighbor_state_var[i], self.ctrl_var[i], self.costate[i],
                                              self.waypoints[i], self.aux_var[i]],
                                             [self.dHx[i]])
            ...
            self.dGu[i] = jacobian(self.path_Hamil_Quad[i], self.ctrl_var[i]).T
            self.dGu_fn[i] = casadi.Function('dGu' + str(i),
                                             [self.neighbor_state_var[i], self.ctrl_var[i], self.costate[i],
                                              self.waypoints[i], self.aux_var[i]],
                                             [self.dGu[i]])
        ...
        for i in range(self.num_uav):
            ...
            self.M_l_fn[i] = casadi.Function('M_l' + str(i), [self.state_var[i], self.ctrl_var[i], self.waypoints[i], self.aux_var[i]],
                                             [self.M_l[i]])
            self.N_l[i] = jacobian(self.dyn[i], self.ctrl_var[i])
            self.N_l_fn[i] = casadi.Function('N_l' + str(i), [self.state_var[i], self.ctrl_var[i], self.waypoints[i], self.aux_var[i]],
                                             [self.N_l[i]])
            self.C_l[i] = jacobian(self.dyn[i], self.full_aux)
            self.C_l_fn[i] = casadi.Function('C_l' + str(i), [self.state_var[i], self.ctrl_var[i], self.waypoints[i], self.aux_var[i]],
                                             [self.C_l[i]])
        ...
        self.J_x = [None] * self.num_uav
        self.J_x_fn = [None] * self.num_uav
        self.J_u = [None] * self.num_uav
        self.J_u_fn = [None] * self.num_uav
        self.J_c = [None] * self.num_uav
        self.J_c_fn = [None] * self.num_uav

        for i in range(self.num_uav):
            self.J_x[i] = jacobian(self.path_Lagr[i], self.state_var[i]).T
            self.J_x_fn[i] = casadi.Function('J_x' + str(i), [self.neighbor_state_var[i], self.ctrl_var[i], self.costate[i], self.state_var_next[i],
                                                              self.waypoints[i], self.aux_var[i]], [self.J_x[i]])

            self.J_u[i] = jacobian(self.path_Lagr[i], self.ctrl_var[i]).T
            self.J_u_fn[i] = casadi.Function('J_u' + str(i), [self.neighbor_state_var[i], self.ctrl_var[i], self.costate[i], self.state_var_next[i],
                                                              self.waypoints[i], self.aux_var[i]], [self.J_u[i]])

            self.J_c[i] = jacobian(self.path_Lagr[i], self.costate[i]).T
            self.J_c_fn[i] = casadi.Function('J_c' + str(i), [self.neighbor_state_var[i], self.ctrl_var[i], self.costate[i], self.state_var_next[i],
                                                              self.waypoints[i], self.aux_var[i]], [self.J_c[i]])



    def dyn_game_Solver(self, ini_state, horizon, auxvar_value, dt, loaded_traj=[None], gamma=1e-4, print_level=0,
                        eph=1, init_option=0, print_gap = 1e2, waypoints = None, solver_option=None):
        assert hasattr(self, 'state_var'), "Define the state variable first!"
        assert hasattr(self, 'ctrl_var'), "Define the control variable first!"
        assert hasattr(self, 'dyn'), "Define the system dynamics first!"
        assert hasattr(self, 'path_cost'), "Define the running cost function first!"
        ...
        # get waypoint
        waypoint_traj = self.waypoint_traj(horizon, waypoints)

        if init_option == 1:
            time = numpy.array([k for k in range(horizon)])
            ini_tau_ctrl_traj = [None] * self.num_uav
            ini_tau_state_traj = [None] * self.num_uav
            ini_tau_costate_traj = [None] * self.num_uav
            ...
            d_tau_state_traj = [None] * self.num_uav
            d_tau_costate_traj = [None] * self.num_uav
            cost_full = [0] * self.num_uav
            cost_traj = [[] for i in range(self.num_uav)]

            # waypoint_traj = self.get_waypoint(horizon,waypoints)

            for i in range(self.num_uav):
                ini_tau_ctrl_traj[i] = np.zeros((horizon - 1, self.n_ctrl_var[i]))
                ini_tau_ctrl_traj[i][:,0] = np.random.rand(1, horizon - 1)
                ini_tau_ctrl_traj[i][:,1] = -np.random.rand(1, horizon - 1)

                ini_tau_ctrl_traj[i] = np.random.rand(horizon - 1, self.n_ctrl_var[i]) # algorithm 2 initilization
                d_tau_ctrl_traj[i] = np.zeros((horizon - 1, self.n_ctrl_var[i])) # horizon: size of T
            ...
            for tau in range(int(min(eph,1e3))):
                ...
                for i in range(self.num_uav):
                    ...
                    for k in range(horizon - 1):
                        tau_state_traj[i][k + 1, :] = self.dyn_fn[i](tau_state_traj[i][k, :], tau_ctrl_traj[i][k, :]).T
                ...
                for i in range(self.num_uav):
                    ...
                    for k in range(horizon - 2, -1, -1):
                        tau_costate_traj[i][k, :] = self.dHx_fn[i](tau_neighbor_state_traj[i][k, :],
                                                                   tau_ctrl_traj[i][k, :],
                                                                   tau_costate_traj[i][k + 1, :], waypoint_traj[k],auxvar_value[i]).T

                    for k in range(horizon - 1):
                        d_tau_ctrl_traj[i][k, :] = self.dHu_fn[i](tau_neighbor_state_traj[i][k, :],
                                                                  tau_ctrl_traj[i][k, :],
                                                                  tau_costate_traj[i][k, :], waypoint_traj[k], auxvar_value[i]).T
                ...
                for i in range(self.num_uav):
                    ...
                    for k in range(horizon - 1):
                        cost_full[i] = cost_full[i] + self.path_cost_fn[i](tau_neighbor_state_traj[i][k, :],
                                                                           tau_ctrl_traj[i][k, :], waypoint_traj[k], auxvar_value[i])
        ...
        if solver_option == 'd':
            ...
            for tau in range(int(eph)):
                ...
                for i in range(self.num_uav):
                    ...
                    for k in range(horizon - 1):
                        tau_state_traj[i][k + 1, :] = self.dyn_fn[i](tau_state_traj[i][k, :], tau_ctrl_traj[i][k, :]).T # T is transpose
                        #print(tau_state_traj[i][k + 1, :])
                ...
                for i in range(self.num_uav):
                    ...
                    cost_full[i] = cost_full[i] + self.final_cost_fn[i](tau_neighbor_state_traj[i][k, :],
                                                            auxvar_value[i])
                    cost_traj[i].append(cost_full[i])
            ...
            for i in range(self.num_uav):
                ...
                for k in range(horizon - 1):
                    ...
                    opt_state_traj[i][k + 1, :] = self.dyn_fn[i](opt_state_traj[i][k], opt_ctrl_traj[i][k]).T
            ...
            for i in range(self.num_uav):
                ...
                for k in range(horizon - 2, -1, -1):
                    opt_costate_traj[i][k,:] = self.dHx_fn[i](opt_neighbor_state_traj[i][k], opt_ctrl_traj[i][k],
                                   opt_costate_traj[i][k + 1], waypoint_traj[k], auxvar_value[i]).T
            ...
            for i in range(self.num_uav):
                ini_tau_ctrl_traj[i] = np.random.rand(horizon - 1, self.n_ctrl_var[i])
                ini_tau_costate_traj[i] = np.random.rand(horizon, self.n_state_var[i])
                if init_option == 0:
                    if loaded_traj['control_traj_opt'][i].shape == ini_tau_ctrl_traj[i].shape:
                        ini_tau_ctrl_traj[i] = loaded_traj['control_traj_opt'][i]
                        # print('his_traj' + str(i) + ' loaded')
                    ...
                    # if loaded_traj['costate_traj_opt'][i].shape == ini_tau_costate_traj[i].shape:
                    #     ini_tau_costate_traj[i] = loaded_traj['costate_traj_opt'][i]
                    # else:
                    #     print('Loaded costate Error')
                ...
                tau_neighbor_state_traj[i] = np.zeros((horizon, self.n_neighbor_state_var[i]))
                tau_ctrl_traj[i] = ini_tau_ctrl_traj[i]
                tau_costate_traj[i] = ini_tau_costate_traj[i]
                tau_state_traj[i][0, :] = ini_state[i]

                d_tau_costate_traj[i] = np.zeros((horizon, self.n_state_var[i]))
                d_tau_state_traj[i] = np.zeros((horizon, self.n_state_var[i]))
                d_tau_ctrl_traj[i] = np.zeros((horizon - 1, self.n_ctrl_var[i]))
            ...
            for tau in range(int(eph)):
                ...
                for i in range(self.num_uav):
                    for k in range(horizon - 1):
                        d_tau_costate_traj[i][k, :] = self.J_x_fn[i](tau_neighbor_state_traj[i][k, :], tau_ctrl_traj[i][k, :], tau_costate_traj[i][k, :],
                                                                   tau_neighbor_state_traj[i][k+1,:], waypoint_traj[k], auxvar_value[i]).T

                        d_tau_state_traj[i][k, :] = self.J_u_fn[i](tau_neighbor_state_traj[i][k, :], tau_ctrl_traj[i][k, :], tau_costate_traj[i][k, :],
                                                                   tau_neighbor_state_traj[i][k+1, :], waypoint_traj[k], auxvar_value[i]).T

                        d_tau_ctrl_traj[i][k, :] = self.J_c_fn(tau_neighbor_state_traj[i][k, :], tau_ctrl_traj[i][k, :], tau_costate_traj[i][k, :],
                                                                   tau_neighbor_state_traj[i][k+1, :], waypoint_traj[k], auxvar_value[i]).T
                ...
                for i in range(self.num_uav):
                    # update costate
                    tau_costate_traj[i] = tau_costate_traj[i] + adaptive_gain * gamma * d_tau_costate_traj[i]

                    #update ctrl
                    tau_ctrl_traj[i] = tau_ctrl_traj[i] - adaptive_gain * gamma * d_tau_ctrl_traj[i]

                    # update state
                    tau_state_traj[i] = tau_state_traj[i] - adaptive_gain * gamma * d_tau_state_traj[i]
                ...
                #    adaptive_gain = (1e3)/ (1e3+tau)
                    ...
                    print(tau, adaptive_gain, cost_full)
                    sio.savemat('data/u_gap.mat',
                               {'d_tau_ctrl_traj': d_tau_ctrl_traj,'d_tau_state_traj':d_tau_state_traj,'d_tau_costate_traj':d_tau_costate_traj})
            ...
            for i in range(self.num_uav):
                ...
                # ini_tau_ctrl_traj[i] = sample_init_ctrl
                tau_state_traj[i] = np.zeros((horizon, self.n_state_var[i]))
                ini_tau_state_traj[i] = np.zeros((horizon, self.n_state_var[i]))
                tau_costate_traj[i] = np.zeros((horizon, self.n_state_var[i]))
                tau_neighbor_state_traj[i] = np.zeros((horizon, self.n_neighbor_state_var[i]))
                tau_ctrl_traj[i] = ini_tau_ctrl_traj[i]
                tau_state_traj[i][0, :] = ini_state[i]

            # initial neighbor trajectory
            for i in range(self.num_uav):
                # compute state trajectory
                for k in range(horizon - 1):
                    ini_tau_state_traj[i][k + 1, :] = self.dyn_fn[i](tau_state_traj[i][k, :],
                                                                 tau_ctrl_traj[i][k, :]).T  # T is transpose
            ...
            for tau in range(int(eph)):
                ...
                for i in range(self.num_uav):
                    # compute state trajectory
                    for k in range(horizon - 1):
                        tau_state_traj[i][k + 1, :] = self.dyn_fn[i](tau_state_traj[i][k, :],
                                                                     tau_ctrl_traj[i][k, :]).T  # T is transpose
                        # print(tau_state_traj[i][k + 1, :])

                # Acquire neighbor trajectories (No communication thus no updates for neighbor traj)
                for i in range(self.num_uav):
                    tau_neighbor_state_traj[i] = []
                    for j in self.neighbors[i]:
                        if len(tau_neighbor_state_traj[i]) == 0:
                            ...
                            if j != i:
                                tau_neighbor_state_traj[i] = np.hstack((tau_neighbor_state_traj[i], ini_tau_state_traj[j]))
                            else:
                                tau_neighbor_state_traj[i] = np.hstack((tau_neighbor_state_traj[i], tau_state_traj[j]))
                ...
                for i in range(self.num_uav):
                    ...
                    for k in range(horizon - 2, -1, -1):
                        tau_costate_traj[i][k, :] = self.dHx_fn[i](tau_neighbor_state_traj[i][k, :],
                                                                   tau_ctrl_traj[i][k, :],
                                                                   tau_costate_traj[i][k + 1, :], waypoint_traj[k],
                                                                   auxvar_value[i]).T
                ...
                for i in range(self.num_uav):
                    ...
                    for k in range(horizon - 1):
                        cost_full[i] = cost_full[i] + self.path_cost_fn[i](tau_neighbor_state_traj[i][k, :],
                                                                           tau_ctrl_traj[i][k, :], waypoint_traj[k],
                                                                           auxvar_value[i])
                    cost_full[i] = cost_full[i] + self.final_cost_fn[i](tau_neighbor_state_traj[i][k, :],
                                                                        auxvar_value[i])
                    cost_traj[i].append(cost_full[i])
    ...
    def dyn_game_Solver_new(self, ini_state, horizon, auxvar_value, dt, loaded_traj=[None], gamma=1e-4, print_level=0,
                        eph=1, init_option=0, print_gap = 1e2, waypoints = None, solver_option=None):
        assert hasattr(self, 'state_var'), "Define the state variable first!"
        assert hasattr(self, 'ctrl_var'), "Define the control variable first!"
        assert hasattr(self, 'dyn'), "Define the system dynamics first!"
        ...
        if init_option == 1:
            ...
            d_tau_state_traj = [None] * self.num_uav
            d_tau_costate_traj = [None] * self.num_uav
            tau_costate_traj_save = [[] for i in range(self.num_uav)]
            tau_state_traj_save = [[] for i in range(self.num_uav)]
            tau_ctrl_traj_save = [[] for i in range(self.num_uav)]
            cost_full = [0] * self.num_uav
            cost_traj = [[] for i in range(self.num_uav)]
            ...
            for i in range(self.num_uav):
                ...
                ini_tau_ctrl_traj[i] = 1 * np.random.rand(horizon - 1, self.n_ctrl_var[i]) # algorithm 2 initilization
                d_tau_ctrl_traj[i] = np.zeros((horizon - 1, self.n_ctrl_var[i])) # horizon: size of T
        ...
        if solver_option == 'd':
            ...
            for tau in range(int(eph)):
                ...
                for i in range(self.num_uav):
                    ...
                    tau_ctrl_traj_save[i] += [tau_ctrl_traj[i]]
                    tau_state_traj_save[i] += [tau_state_traj[i]]
                    tau_costate_traj_save[i] += [tau_costate_traj[i]]
                ...
                #print(d_tau_ctrl_traj)
                #exit()
                opt_sol = {"state_traj_save": tau_state_traj_save,
                           "control_traj_save": tau_ctrl_traj_save,
                           "costate_traj_save": tau_costate_traj_save,
                           "state_traj_opt": tau_state_traj,
                           "control_traj_opt": tau_ctrl_traj,
                           "costate_traj_opt": tau_costate_traj,
                           'auxvar_value': auxvar_value,
                           "time": time,
            ...
            for i in range(self.num_uav):
                ini_tau_ctrl_traj[i] = np.random.rand(horizon - 1, self.n_ctrl_var[i])
                ini_tau_costate_traj[i] = 0.5*np.random.rand(horizon, self.n_state_var[i])
                if init_option == 0:
                    if loaded_traj['control_traj_opt'][i].shape == ini_tau_ctrl_traj[i].shape:
                        ini_tau_ctrl_traj[i] = loaded_traj['control_traj_opt'][i]
                        # print('his_traj' + str(i) + ' loaded')
            ...
            for tau in range(int(eph)):
                ...
                for i in range(self.num_uav):
                    for k in range(horizon - 1):
                        d_tau_costate_traj[i][k, :] = self.J_c_fn[i](tau_neighbor_state_traj[i][k, :],
                                                                     tau_ctrl_traj[i][k, :], tau_costate_traj[i][k, :],
                                                                     tau_state_traj[i][k + 1, :],
                                                                     waypoint_traj[k], auxvar_value[i]).T

                        d_tau_state_traj[i][k, :] = self.J_x_fn[i](tau_neighbor_state_traj[i][k, :],
                                                                   tau_ctrl_traj[i][k, :], tau_costate_traj[i][k, :],
                                                                   tau_state_traj[i][k + 1, :],
                                                                   waypoint_traj[k], auxvar_value[i]).T

                        d_tau_ctrl_traj[i][k, :] = self.J_u_fn[i](tau_neighbor_state_traj[i][k, :], tau_ctrl_traj[i][k, :],
                                                               tau_costate_traj[i][k, :],
                                                               tau_state_traj[i][k + 1, :], waypoint_traj[k],
                                                               auxvar_value[i]).T

                # adaptive set the gamma
            ...
            sol_ctrl_traj = [None] * self.num_uav

            # ini_ctrl_traj = [None] * self.num_uav
            # tau_ctrl_traj = [None] * self.num_uav
            ...
            for i in range(self.num_uav):
                var_state_traj[i] = [None] * horizon
                tau_state_traj[i] = [None] * horizon
                var_costate_traj[i] = [None] * horizon
                var_state_traj[i][0] = ini_state[i]

                # # initial ctrl_traj
                # ini_ctrl_traj[i] = tau_ctrl_traj[i]
                # tau_ctrl_traj[i] = ini_ctrl_traj[i]


            for tau in range(int(eph)):

                w_stac = [[] for i in range(self.num_uav)]
                g_stac = [[] for i in range(self.num_uav)]
                ...
                # neighbors dynamics
                for i in range(self.num_uav):
                    tau_state_traj[i][0] = var_state_traj[i][0]
                    for k in range(horizon - 1):
                    ...
                    for k in range(horizon):
                        ...
                        for j in self.neighbors[i]:
                            if i == j:
                                ...
                                var_neighbor_vec += [tau_state_traj[j][k]]
                        var_neighbor_state_traj[i][k] = vertcat(*var_neighbor_vec)
                ...
                for i in range(self.num_uav):
                    ...
                    for k in range(horizon - 2, -1, -1):
                        var_costate_traj[i][k] = self.dHx_fn[i](var_neighbor_state_traj[i][k], var_ctrl_traj[i][k],
                                                                var_costate_traj[i][k + 1], waypoint_traj[k],
                                                                auxvar_value[i])

                    for k in range(horizon - 1):
                        g_stac[i] += [self.dHu_fn[i](var_neighbor_state_traj[i][k], var_ctrl_traj[i][k],
                                                  var_costate_traj[i][k + 1],
                                                  waypoint_traj[k], auxvar_value[i])]
                ...
                for i in range(self.num_uav):
                    ...
                    g = vertcat(*g_stac[i])
                    #opts = {'error_on_fail': True, 'print_iteration': True}
                    opts = {'nlpsol':'ipopt', 'error_on_fail': False}
                    #opts = {'nlpsol':'ipopt', 'ipopt.print_level': print_level, 'ipopt.sb': 'yes', 'print_time': print_level}
                    prob = {'x': w, 'g': g}
                    #solver = rootfinder('G', 'newton', prob, opts)
                    solver = rootfinder('G', 'nlpsol', prob, opts)
                    # print('Start solving the trajectory:')
                    ini_sol_stac = np.stack(ini_sol_ctrl_traj[i])
                    ...
                    sol_ctrl_traj[i] = np.reshape(full_traj, (horizon-1, -1))
                    #sol_costate_traj = [np.reshape(decomposed_traj[2][i], (horizon, -1)) for i in range(self.num_uav)]
                ...
                for i in range(self.num_uav):
                    ...
                    tau_ctrl_traj[i] = opt_ctrl_traj[i]
                    ini_sol_ctrl_traj[i] = tau_ctrl_traj[i]

                    tau_ctrl_traj_save[i] += [tau_ctrl_traj[i]]
                    tau_state_traj_save[i] += [opt_state_traj[i]]
                    tau_costate_traj_save[i] += [tau_costate_traj[i]]
                ...
                for i in range(self.num_uav):
                    cost_full[i] = 0
                    for k in range(horizon - 1):
                        cost_full[i] = cost_full[i] + self.path_cost_fn[i](opt_neighbor_state_traj[i][k],
                                                                           opt_ctrl_traj[i][k], waypoint_traj[k],
                                                                           auxvar_value[i])
                    cost_full[i] = cost_full[i] + self.final_cost_fn[i](opt_neighbor_state_traj[i][k],
                                                                        auxvar_value[i])
                    cost_traj[i].append(cost_full[i])
    ...
    def D_PMP_Solver(self, horizon, auxvar_value, gamma=1e-4, loaded_traj=[None], print_level=0, eph=1,
                     solover_option=0, waypoints=None):
        ...
        if solover_option == 0:
            ...
            for k in range(horizon - 1):
                ...
                for i in range(self.num_uav):
                    ...
                    M_l_V[k][i] = self.M_l_fn[i](curr_state_traj[i][k, :],
                                                 curr_ctrl_traj[i][k, :], waypoint_traj[k],
                                                 auxvar_value[i]) # 8a. dif dyn/x
                    N_l_V[k][i] = self.N_l_fn[i](curr_state_traj[i][k, :],
                                                 curr_ctrl_traj[i][k, :], waypoint_traj[k],
                                                 auxvar_value[i]) # 8a. dif dyn/u
                    C_l_V[k][i] = self.C_l_fn[i](curr_state_traj[i][k, :],
                                                 curr_ctrl_traj[i][k, :], waypoint_traj[k],
                                                 auxvar_value[i]) # 8a. dif dyn/costate


                    # diff input
                    M_u_V[k][i] = self.M_u_fn[i](curr_neighbor_state_traj[i][k, :],
                                                 curr_ctrl_traj[i][k, :],
                                                 curr_costate_traj[i][k, :], waypoint_traj[k], auxvar_value[i]) # 8b. dif dHu/x
                    N_u_V[k][i] = self.N_u_fn[i](curr_neighbor_state_traj[i][k, :],
                                                 curr_ctrl_traj[i][k, :],
                                                 curr_costate_traj[i][k, :], waypoint_traj[k], auxvar_value[i]) # 8b. dif dHu/u
                    ...
                    S_u_V[k][i] = self.S_u_fn[i](curr_neighbor_state_traj[i][k, :],
                                                 curr_ctrl_traj[i][k, :],
                                                 curr_costate_traj[i][k, :], waypoint_traj[k], auxvar_value[i]) # 8b. dif dHu/costate
                    C_u_V[k][i] = self.C_u_fn[i](curr_neighbor_state_traj[i][k, :],
                                                 curr_ctrl_traj[i][k, :],
                                                 curr_costate_traj[i][k, :], waypoint_traj[k], auxvar_value[i]) # 8b. dif dh/full_aux

                    # diff state
                    M_x_V[k][i] = self.M_x_fn[i](curr_neighbor_state_traj[i][k, :],
                                                 curr_ctrl_traj[i][k, :],
                                                 curr_costate_traj[i][k, :], waypoint_traj[k], auxvar_value[i]) # 8c. dif dHx/x

                    N_x_V[k][i] = self.N_x_fn[i](curr_neighbor_state_traj[i][k, :],
                                                 curr_ctrl_traj[i][k, :],
                                                 curr_costate_traj[i][k, :], waypoint_traj[k], auxvar_value[i]) # 8c. dif dHx/u
                    ...
                    S_x_V[k][i] = self.S_x_fn[i](curr_neighbor_state_traj[i][k, :],
                                                 curr_ctrl_traj[i][k, :],
                                                 curr_costate_traj[i][k, :], waypoint_traj[k], auxvar_value[i]) # 8c. dif dHx/costate
                    C_x_V[k][i] = self.C_x_fn[i](curr_neighbor_state_traj[i][k, :],
                                                 curr_ctrl_traj[i][k, :],
                                                 curr_costate_traj[i][k, :], waypoint_traj[k], auxvar_value[i]) # 8c. dif dHx/full_aux
            ...
            for i in range(self.num_uav):
                ...
                stack_M = []
                for k in range(horizon - 1):
                    stack_M += [M_l_V[k][i]] # 8a. dif dyn/x
                ...
                # print('B_M:','shape:',B_M[i].shape,'rank:',matrix_rank(B_M[i]))
                # print('B_N:', 'shape:', B_N[i].shape, 'rank:', matrix_rank(B_N[i]))
                # print('B_S:', 'shape:', B_S[i].shape, 'rank:', matrix_rank(B_S[i]))
                # print('B_C:','shape:',B_C[i].shape,'rank:',matrix_rank(B_C[i]))
                # print('B_Q:', 'shape:', B_Q[i][0].shape, 'rank:', matrix_rank(B_Q[i][0]))
            ...
            # print(matrix_rank(A_compact))
            C_compact = np.vstack(C_compact_stack)
            # print(matrix_rank(C_compact))
    ...
    def get_waypoint(self, horizon, waypoints):
        waypoint_traj = [[]]
        for uav_index in range(self.num_uav):
            waypoint_num = len(waypoints)
            for t in range(waypoint_num):
                timetag = int(horizon/waypoint_num)
                if t != waypoint_num-1:
                    waypoint_traj[t*timetag:t*timetag+timetag] = [waypoints[t] for i in range(timetag)]
                else:
                    waypoint_traj[t*timetag:horizon] = [waypoints[t] for i in range(horizon-t*timetag)]

        return waypoint_traj

    def waypoint_traj(self, horizon, waypoints):
        ...
        for pos in waypoints:
            pos_x.append(pos[0])
            pos_y.append(pos[1])
        poly = np.polyfit(pos_x, pos_y, waypoint_num-1)
        traj_generate = np.poly1d(poly)

        # fig = plt.figure()
        x = np.linspace(pos_x[0], pos_x[-1], horizon)
        y = np.polyval(poly,x)
        # plt.plot(x,y)


        for i in range(horizon):
            waypoint_traj.append([x[i], y[i]])
            # print(waypoint_traj)
        return waypoint_traj