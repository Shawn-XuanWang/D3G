...
base_parameter = [[1, 50, 0.2, 0.4, 1, 0, 0]] * 4
...
# To Be Filled
trajectories = [None] * 3
data = sio.loadmat('data/uav_formation_demos.mat')
...
true_parameter = [true_parameter[i][1:] for i in range(num_uav)]
print('New true parameter:')
print(true_parameter)
# print('base parameter:')
# print(base_parameter)

def compute_gradient(d, k, num_uav, uav_adj_matrix, wthrust_set, trajectories, curr_sys_traj, dt, base_parameter, current_parameter):
...
if __name__ == '__main__':
    ...
    for tr in range(1):  # trial loop
        ...
        for k in range(int(1e3)):  # iteration loop (or epoch loop)
            loss = 0
            ...
            curr_sys_traj = pool.starmap(compute_gradient, [(d, k, num_uav, uav_adj_matrix, wthrust_set, trajectories, curr_sys_traj, dt, base_parameter, current_parameter) for d in range(n_demo)])
            ...
            for d in range(n_demo):
                ...
                #sio.savemat('data/check.mat', {'curr': curr_sys_traj})
                for i in range(num_uav):
                ...
                for i in range(num_uav):
                    ...
                    for t in range(demo_horizon - 1):
                        ...
                        for j in range(num_uav):
                            dp[i] = dp[i] + np.matmul(dldx_traj[j][t, :], dxdtheta_current[j]) + np.matmul(dldu_traj[j][t, :],
                                                                                                       dudtheta_current[j])
