...
#demo 3
ini_r_I = [[0.7403, 0.3236], [2.5098, 1.2585], [4.1842, 0.0630], [2.2504, 0.0228-1]]
#demo 4
...
#                  1  2  3    4   5    6  7   8  9
true_parameter = [[1, 1, 50, 0.2, 0.4, 1.6, 10, 10]] *num_uav
base_parameter = [[1, 1, 50, 0.2, 0.4, 1.6, 10, 10]] *num_uav

restart = 0
# [1 Jx, 4 mass, 5 w_F_v0, 6 w_r_formation0, 7 w_v_formation0, 8 uav_dist0, 9 w_uav_collision0]
# --------------------------- load environment ----------------------------------------
uavswarm = [XEnv.UAV_formation_transition_diamond_2D() for _ in range(num_uav)]
...
if restart == 1:
    traj = uav_game.dyn_game_Solver(ini_state=uav_ini_state, horizon=formation_horizon, auxvar_value=base_parameter,
                             init_option=1, print_gap=100, eph=2e3, gamma=1e-6, dt=dt)
    ...
    traj = uav_game.dyn_game_Solver(ini_state=uav_ini_state, horizon=formation_horizon, auxvar_value=true_parameter,
                                init_option=0, eph = 3e3, print_gap=1e2, gamma=1e-4, solver_option='cs', loaded_traj=load_traj, dt=dt)

    sio.savemat('data/uav_formation_demos3.mat', {'trajectories': traj,
                                  'dt': dt,
                                   'true_parameter': true_parameter})
uavswarm[0].play_multi_animation_tran(uavswarm=uavswarm, wing_len=0.2, dt=dt, state_traj=traj['state_traj_opt'],save_option=0)
