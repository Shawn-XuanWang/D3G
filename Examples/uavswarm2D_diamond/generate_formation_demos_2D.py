...
#                  1  2  3    4   5    6  7   8  9
true_parameter = [[1, 1, 50, 0.2, 0.4, 2, 10, 10]] *num_uav
base_parameter = [[1, 1, 50, 0.2, 0.4, 2, 10, 10]] *num_uav

restart = 0
# [1 Jx, 4 mass, 5 w_F_v0, 6 w_r_formation0, 7 w_v_formation0, 8 uav_dist0, 9 w_uav_collision0]
# --------------------------- load environment ----------------------------------------
uavswarm = [XEnv.UAV_formation_diamond_2D() for _ in range(num_uav)]
...
if restart == 1:
    traj = uav_game.dyn_game_Solver(ini_state=uav_ini_state, horizon=formation_horizon, auxvar_value=base_parameter,
                             init_option=1, print_gap=100, eph=5e3, gamma=1e-6, dt=dt)
